...
public class Mage extends Playable {
	...
	@Override
	public void attackAction(float delta, int direction) {
		Projectile prj = null;
		switch (direction) {
		case Indentifiers.DIRECTION_SOUTH:
			prj = new Fireball(Indentifiers.DIRECTION_SOUTH, agility, strength, new int[] { pos[0], pos[1] });
			prj.getPos()[0] += currentAni.getKeyFrame(0).getRegionWidth() / 4;
			prj.getPos()[1] -= prj.getSize()[1];
			break;
		case Indentifiers.DIRECTION_NORTH:
			prj = new Fireball(Indentifiers.DIRECTION_NORTH, agility, strength, new int[] { pos[0], pos[1] });
			prj.getPos()[0] += currentAni.getKeyFrame(0).getRegionWidth() / 4;
			prj.getPos()[1] += currentAni.getKeyFrame(0).getRegionWidth() * 1.5;
			break;
		case Indentifiers.DIRECTION_EAST:
			prj = new Fireball(Indentifiers.DIRECTION_EAST, agility, strength, new int[] { pos[0], pos[1] });
		...
		case Indentifiers.DIRECTION_WEST:
			prj = new Fireball(Indentifiers.DIRECTION_WEST, agility, strength, new int[] { pos[0], pos[1] });
			prj.getPos()[1] += currentAni.getKeyFrame(0).getRegionHeight() / 4;
